...
export const BADGE_DEFS: Record<string, { label: string; icon: string; desc: string; color: string }> = {
  top_hacker:      { label: "Top Hacker",      icon: "👑", desc: "Highest daily score",                color: "border-yellow-400/40 bg-yellow-400/10 text-yellow-400" },
  phishing_master: { label: "Phishing Master",  icon: "🎣", desc: "10+ correct phishing answers",       color: "border-blue-400/40 bg-blue-400/10 text-blue-400" },
  speed_runner:    { label: "Speed Runner",     icon: "⚡", desc: "Lightning-fast average response",    color: "border-primary/40 bg-primary/10 text-primary" },
  defender_pro:    { label: "Defender Pro",     icon: "🛡️", desc: "Defense score over 300",            color: "border-green-400/40 bg-green-400/10 text-green-400" },
};
...
export function BadgeDisplay({ badges, size = "sm", showLabels = false, animate = true }: BadgeDisplayProps) {
      ...
      {badges.map((badgeId, i) => {
        ...
        const badge = (
          <motion.div
            ...
            className={`inline-flex items-center gap-1 rounded-full border font-mono font-bold ${sizeClasses[size]} ${def.color}`}
          >
            <span>{def.icon}</span>
        ...
        return (
          <Tooltip key={badgeId}>
            ...
            <TooltipContent side="top" className="text-xs">
              <p className="font-bold">{def.icon} {def.label}</p>
...
export function StreakTitle({ streakDays, className = "", animate = true }: StreakTitleProps) {
...
export function IdentityLine({ username, rankTier, hackerType, streakDays, badges, isTopHacker }: IdentityLineProps) {
  ...
  return (
    <div className="flex items-center flex-wrap gap-1.5 text-xs font-mono">
      ...
      <span className="text-muted-foreground">•</span>
      <span className={hackerType === "attacker" ? "text-red-400" : "text-primary"}>{role}</span>
      {streakTitle && (
        <>
          <span className="text-muted-foreground">•</span>
