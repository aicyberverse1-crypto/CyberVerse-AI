...
router.post("/ai/hint", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const cutoff = new Date(Date.now() - 1); // used as a non-null reference for the atomic deduction

  // Read user to check balance before deducting
  ...
  if (user.hintPoints < cost) {
    res.status(402).json({
      error: `Not enough hint points. Need ${cost}, have ${user.hintPoints}. Earn more by answering correctly or claiming your daily bonus!`,
    });
    return;
  }

  // Atomic deduction — prevents two simultaneous hint requests from double-spending
  const [updated] = await db
    .update(usersTable)
    .set({ hintPoints: sql`${usersTable.hintPoints} - ${cost}` })
    .where(and(eq(usersTable.id, userId), sql`${usersTable.hintPoints} >= ${cost}`))
    .returning();

  if (!updated) {
    res.status(402).json({ error: "Not enough hint points." });
    return;
  }
  ...
  // AI Memory: fetch recent performance to tailor hints
  ...
  const memoryContext = weakAreas.length > 0
    ...
    ? `\n\nStudent performance note: Solid track record (${recentScores.filter(s => s.score > 0).length}/${recentScores.length} correct). Give a nuanced hint that challenges deeper thinking.`
    : "";
...
- Never reveal the correct answer directly`,
    ...
    res.json({ hint, hintPointsCost: cost, hintPointsRemaining: updated.hintPoints });
  } catch (err) {
    // Refund the hint points if the AI call fails
    await db.update(usersTable)
      .set({ hintPoints: sql`${usersTable.hintPoints} + ${cost}` })
      .where(eq(usersTable.id, userId));
    res.status(503).json({ error: "AI hint service is temporarily unavailable. Your hint points have been refunded." });
  }
});
...
Keep responses concise (3-5 sentences), educational, and practical. Use concrete examples when helpful.`,
    ...
    res.status(503).json({ error: "AI assistant is temporarily unavailable. Please try again in a moment." });
  }
});
...
Make each story feel current, realistic, and educational. Vary severity levels and categories.`,
    ...
    // Return fallback signal so the client can show its built-in fallback data
    res.status(503).json({ error: "AI news service temporarily unavailable.", useFallback: true });
  }
});
